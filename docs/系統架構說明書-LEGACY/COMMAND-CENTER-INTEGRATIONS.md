...
## 八、Google Ads OAuth（`invalid_grant`）

### 8.1 常見原因

| 現象 | 原因 |
|------|------|
| `invalid_grant` + Refresh Token 空白 | 後台儲存時未貼上 Refresh Token |
| `Token has been expired or revoked` | 舊 Refresh Token 已撤銷，或 OAuth 用戶端 Secret 已重設 |
| Client ID 與 Token 不符 | Playground 用 A 專案 Client，後台填 B 專案 |

### 8.2 重新取得 Refresh Token（建議）

1. [GCP 憑證](https://console.cloud.google.com/apis/credentials) → 你的 OAuth 用戶端 → **已授權重新導向 URI** 新增：
   - `http://localhost:8766/callback`
   - （可選）`https://developers.google.com/oauthplayground`
2. `.env.local` 填入 `GOOGLE_ADS_CLIENT_ID`、`GOOGLE_ADS_CLIENT_SECRET`（與後台相同）
3. 本機執行：
...
4. 用 **擁有 Ads 帳戶 8702788584 權限** 的 Google 帳號登入 → 複製終端機輸出的 Refresh Token
5. 後台 **整合中心 → Google Ads** → 貼上 Refresh Token → **啟動連線**

或一次寫入 DB：

```bash
# .env.local 需含 GOOGLE_ADS_* 全套（含新 REFRESH_TOKEN）
...
### 8.3 OAuth Playground 備援

1. [OAuth 2.0 Playground](https://developers.google.com/oauthplayground/) → 齒輪 → **Use your own OAuth credentials**
2. Step 1 Scope：`https://www.googleapis.com/auth/adwords`
3. 授權後 Step 2 複製 **Refresh token** 貼到後台
...
| 腳本 | 用途 |
|------|------|
| `scripts/google-ads-oauth-grant.mjs` | 取得 Ads Refresh Token |
| `scripts/upsert-google-ads-from-env.mjs` | 將 GOOGLE_ADS_* 寫入 DB 並驗證 |
| `scripts/probe-google-ads-oauth.mjs` | 診斷 DB / OAuth 狀態 |
